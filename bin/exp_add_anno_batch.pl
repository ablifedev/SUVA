...
if(!defined($opts{exp}) || !defined($opts{anno}))
{
	...
	Usage:perl $0
		...
		-o                 outfile sufix                   option,default is [exp].anno
...
my @exps = split(/,/,$exp_files);

open(EXP,"$exps[0]")||die "Can't open $exps[0]\n";
while(<EXP>){
	chomp;
	s/\r//g;
	my @line=split(/\t/);
...
foreach my $exp_file (@exps){
	next if $exp_file eq "";
	my $out_file=$exp_file.".anno";
	$out_file=$exp_file.$opts{o} if defined($opts{o});
