...
for chr in chr_lines:
    chr=chr.strip()
    chr_list.append(chr)
    hash1[chr]=HTSeq.GenomicArrayOfSets([chr], stranded=False)


hashgene_s = {}
hashgene_e = {}
hashgene_s["intergenic"]=0
hashgene_e["intergenic"]=0
#使用HTSeq载入gff文件
f1=open(gff_file,"r")
while 1:
    line=f1.readline()
    if line=="":
        ...
        if line.startswith("#"):
            ...
            if items[2] in genes and items[0] in chr_list:
                ...
                hashgene_s[gene_name] = start
                hashgene_e[gene_name] = end
                gene_iv = HTSeq.GenomicInterval(chr, int(start) - 1, int(end), ".")
...
while 1:
    ...
    if line=="":
        ...
        items = line.split("\t")
        start = int(items[1]) - 25000
        if start < 1:
            start = 1
        end = int(items[2]) + 25000
        chr = items[0]
        ...
        if flag==0:
            tmp_1="error"
            print("error")
        f3.write(line.strip() + "\t" + tmp_1 + "\t" + str(hashgene_s[tmp_1]) + "\t" + str(hashgene_e[tmp_1]) + "\n")
        # f3.write(line.strip() + "\t" + tmp_1 + "\t" + str(hashgene_s[tmp_1]) + "\t" + str(hashgene_e[tmp_1]) + "\t---".join(tmp) + "\n")
f1.close()
f2.close()
f3.close()
